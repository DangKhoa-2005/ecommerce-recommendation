...
app.post("/users/login", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT id, email, full_name, role, olist_customer_id, password_hash FROM users WHERE email = $1",
      [email]
    );
    ...
    return res.json({
      ...
      user: {
        ...
        role: user.role,
        olist_customer_id: user.olist_customer_id || null
      }
    });
  } catch (err) {
...
app.get("/users/me", authRequired, async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id, email, full_name, role, olist_customer_id, created_at FROM users WHERE id = $1",
      [req.user.id]
    );
